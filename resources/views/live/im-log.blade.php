        ...
        <table class="table table-" style="table-layout:fixed;">
            ...
            @foreach($list as $row)
                <tr>
                    ...
                    <td style="text-align: left;white-space: normal;word-break: break-all;">@if($row->msg_type == 1) {{$row->msg_content->text}} @elseif($row->msg_type == 4) {{$row->msg_content->Data}} @if($row->msg_content->Data == 'REMOVE_MSG')  {{$row->msg_content->Ext}}  @endif @endif</td>
