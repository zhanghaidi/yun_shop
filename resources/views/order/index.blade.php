                            ...
                            <form action="" method="get" class="form-horizontal" id="form1">
                                                ...
                                                <div>
                                                    <div class='input-group'>
                                                        <input type="hidden" id="plugin_id" name="plugin_id"
                                                               value="@if(!empty($list['plugin_id'])) {{$list['plugin_id']}} @else 0 @endif">
                                                        <input type="text" name="search[ambiguous][name]" maxlength="30"
                                                               value="{{array_get($requestSearch,'ambiguous.name','')}}"
                                                               id="saler" class="form-control" readonly/>
                                                        <div class='input-group-btn'>
                                                            <button class="btn btn-default" type="button"
                                                                    onclick="popwin = $('#modal-module-menus-notice').modal();">
                                                                选择商品
                                                            </button>
                                                            <button class="btn btn-danger" type="button"
                                                                    onclick="$('#noticeopenid').val('');$('#saler').val('');$('#saleravatar').hide()">
                                                                清除选择
                                                            </button>
                                                        </div>
                                                    </div>
                                                    <div id="modal-module-menus-notice" class="modal fade"
                                                         tabindex="-1">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-7 col-lg-9 col-xs-12">
                                        ...
                                        @section('export')
                                            <button type="submit" name="export" value="1" id="export"
                                                    class="btn btn-info">导出
                                                Excel
                                            </button>
                                            @if(app('plugins')->isEnabled('team-dividend'))
                                                <button type="submit" name="direct_export" value="1" id="direct-export"
                                                        class="btn btn-info">导出
                                                    直推 Excel（经销商）
                                                </button>
                                            @endif
                                ...
                                @foreach( $order['has_many_order_goods'] as $order_goods_index => $order_goods)
                                    <tr class='trbody'>
                                        ...
                                        <td class="top" valign='top' style="font-size: 16px;color: #AEB9C0">
                                            ...
                                            @endif
                                            <br/><span
                                                    style="font-size: 15px;color: #AEB9C0">{{$order_goods['goods_sn']}}</span>
