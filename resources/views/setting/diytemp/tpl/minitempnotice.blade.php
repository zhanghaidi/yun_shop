...
{{--                    <option @if(empty($temp['appid'])) selected @endif value="2" >H5网页</option>--}}
        ...
        <div class="form-group miniprogram-div">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <select class="form-control diy-notice" onchange="$('.tm').hide();$('.tm-'+$(this).val()).show()">
                        ...
                        @foreach(\app\common\modules\template\MiniTemplate::current()->getItems() as $item)
...
<script language='javascript'>
    ...
    function restempoption() {
        var url = "{!! yzWebUrl('setting.small-program.returnJson') !!}";
        $.ajax({
            "url": url,
            success: function (ret) {
    ...
    $('select[name="temp[sel_h5_mini]"]').change(function () {
        if($(this).val() == 1){
            $('.news_link-div').hide()
