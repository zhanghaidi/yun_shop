...
<style>
.el-checkbox__input.is-disabled.is-checked+ span.el-checkbox__label{color: #409EFF;border-color:#409EFF}
.el-checkbox.is-bordered.is-disabled.is-checked {border-color: #409EFF;}
.el-checkbox__input.is-disabled.is-checked .el-checkbox__inner, .el-checkbox__input.is-indeterminate .el-checkbox__inner {background-color: #409EFF;border-color: #409EFF;}
</style>
        ...
        <template>
            <div class="second-list">
                <div class="rightlist-head">
                    <div style="margin-left:10px;font-size:16px;display:inline-block">
                        商品列表
                    </div>
                    <a href="{{yzWebUrl('plugin.supplier.supplier.controllers.goods.goods-operation.add')}}">
                        <el-button icon="el-icon-plus" type="primary" size="small">发布商品</el-button>
                    </a>
                </div>
                        ...
                        <div>
                            <template>
                                ...
                                <el-table :data="goods_list" style="width: 100%" :class="table_loading==true?'loading-height':''" v-loading="table_loading">
                                    ...
                                    <el-table-column prop="real_sales" label="销量" width="70" align="center"></el-table-column>
                                    ...
                                    <el-table-column label="操作" width="300" align="center">
                                        <template slot-scope="scope">
                                            <div class="table-option">
                                                ...
                                                <a :href="scope.row.copy_link">
                                                    复制商品
                                                </a>
                                                &nbsp;&nbsp;
                                                <a :href="'{{ yzWebFullUrl('plugin.supplier.supplier.controllers.goods.goods-operation.edit', array('id' => '')) }}'+[[scope.row.id]]" target="_blank">
                                                编辑
                                                </a>&nbsp;&nbsp;
                                            ...
                                            <div>
                                                <el-checkbox border disabled size="mini" v-model.number="scope.row.is_new" :true-label="1" :false-label="0" @change="setProperty(scope.row.id,scope.$index,'is_new')">新品</el-checkbox>
                                                <el-checkbox border disabled size="mini" v-model.number="scope.row.is_hot" :true-label="1" :false-label="0" @change="setProperty(scope.row.id,scope.$index,'is_hot')">热卖</el-checkbox>
                                                <el-checkbox border disabled size="mini" v-model.number="scope.row.is_recommand" :true-label="1" :false-label="0" @change="setProperty(scope.row.id,scope.$index,'is_recommand')">推荐</el-checkbox>
                                                <el-checkbox border disabled size="mini" v-model.number="scope.row.is_discount" :true-label="1" :false-label="0" @change="setProperty(scope.row.id,scope.$index,'is_discount')">促销</el-checkbox>
                                            </div>
    ...
    var app = new Vue({
        ...
        methods: {
            getData() {
                ...
                that.$http.post("{!! yzWebFullUrl('plugin.supplier.supplier.controllers.goods.supplier-goods-list.goods-list') !!}").then(response => {
                    ...
                    if(response.data.result==1){
                        ...
                        that.category_list = response.data.data.catetory_menus.ids;
                        that.catlevel = response.data.data.catetory_menus.catlevel;
            ...
            search(page) {
                ...
                that.table_loading = true;
                that.$http.post("{!! yzWebFullUrl('plugin.supplier.supplier.controllers.goods.supplier-goods-list.goods-search') !!}",json).then(response => {
                    console.log(response);
                    if(response.data.result==1){
                        let arr = [];
            ...
            delOne(id) {
                ...
                that.$confirm('确定删除吗', '提示', {confirmButtonText: '确定',cancelButtonText: '取消',type: 'warning'}).then(() => {
                    that.table_loading = true;
                    that.$http.post("{!! yzWebFullUrl('plugin.supplier.supplier.controllers.goods.goods-operation.delete') !!}",{id:id}).then(response => {
                        console.log(response);
                        if(response.data.result==1){
            ...
            batchDestroy() {
                ...
                that.$confirm('确定删除吗', '提示', {confirmButtonText: '确定',cancelButtonText: '取消',type: 'warning'}).then(() => {
                    ...
                    let json = {ids:ids}
                    that.$http.post("{!! yzWebFullUrl('plugin.supplier.supplier.controllers.goods.goods-operation.batchDelete') !!}",json).then(response => {
                        console.log(response);
                        if(response.data.result==1){
                            that.$message.success('操作成功！');
                            that.is_all_choose = 0;
            ...
            confirmChange(id,type) {
                ...
                that.table_loading = true;
                that.$http.post("{!! yzWebFullUrl('plugin.supplier.supplier.controllers.goods.goods-operation.change') !!}",json).then(response => {
                    console.log(response);
                    if(response.data.result==1){
                        that.$message.success('操作成功！');
                        if (document.all) {
            ...
            confirmChangeSort(id) {
                ...
                that.$http.post("{!! yzWebFullUrl('plugin.supplier.supplier.controllers.goods.goods-operation.sort') !!}",json).then(response => {
                    console.log(response);
                    if(response.data.result==1){
                        that.$message.success('操作成功！');
