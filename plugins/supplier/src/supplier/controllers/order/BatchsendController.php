...
class BatchsendController extends SupplierCommonController
{
    ...
    public function index()
    {
        ...
        if (\Request::isMethod('post')) {
            if ($send_data['express_company_name'] == "顺丰" && $send_data['express_code'] != "shunfeng") {
                return $this->message('上传失败，请重新上传', Url::absoluteWeb('plugin.supplier.supplier.controllers.order.batchsend.index'), 'error');
            }

            if (!$send_data['excelfile']) {
                return $this->message('请上传文件', Url::absoluteWeb('plugin.supplier.supplier.controllers.order.batchsend.index'), 'error');
            }
            ...
            if ($send_data['excelfile']->isValid()) {
                ...
                return $this->message($msg . $this->error_msg, Url::absoluteWeb('plugin.supplier.supplier.controllers.order.batchsend.index'));
            }
        }
        return view('Yunshop\Supplier::supplier.order.batchsend', []);
    }
    ...
    private function handleOrders($values, $send_data)
    {
        foreach ($values as $rownum => $col) {
            ...
            $order = SupplierOrderJoinOrder::select('id', 'order_sn', 'status', 'refund_id')->whereStatus(1)->whereOrderSn($order_sn)->first();
            if (!$order) {
                $this->err_array[] = $order_sn;
                continue;
            }
