...
class OrderTeamAward extends BaseModel
{
    public $table = 'yz_mryt_order_team_award';
    ...
    public function scopeSearch($query, $search)
    {
        ...
        if ($search['log_id']) {
            $query->where('log_id', $search['log_id']);
        }
        if ($search['log_member']) {
            $query->whereHas('hasOneSourceMember', function ($log_member) use ($search) {
                $log_member->select('uid', 'nickname', 'realname', 'mobile', 'avatar')
                    ->where('realname', 'like', '%' . $search['log_member'] . '%')
                    ->orWhere('mobile', 'like', '%' . $search['log_member'] . '%')
                    ->orWhere('nickname', 'like', '%' . $search['log_member'] . '%');
            });
        }
    ...
    {
        return $this->hasOne(Member::class, 'uid', 'log_uid');
    }
