...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("input[name='back']").click(function () {
            location.href = "{!! yzWebUrl('plugin.store-cashier.admin.goods.index') !!}";
        });
    })
    ...
    function formcheck() {
        ...
        if (!numerictype.test($(':input[name="widgets[sale][min_point_deduct]"]').val()) && $(':input[name="widgets[sale][min_point_deduct]"]').val() != '') {
            $('#myTab a[href="#tab_sale"]').tab('show');
            Tip.focus(':input[name="widgets[sale][min_point_deduct]"]', '最少积分抵扣格式错误,只能为非负整数或空.');
            return false;
        }
        if (!numerictype.test($(':input[name="widgets[sale][award_balance]"]').val()) || $(':input[name="widgets[sale][award_balance]"]').val() == '') {
            $('#myTab a[href="#tab_sale"]').tab('show');
            Tip.focus(':input[name="widgets[sale][award_balance]"]', '余额赠送格式错误,只能为非负整数或空.');
            return false;
        }
        ...
        <div class="top">
            <ul class="add-shopnav" id="myTab">
                ...
                @foreach(Config::get('widget.goods') as $key=>$value)
                    ...
                    @endif
                @endforeach
                <li><a href="#tab_store_setting">门店设置</a></li>
            </ul>
        </div>
        <div class="info">
            <div class="panel-body">
                <div class="tab-content">
                    <div class="tab-pane  active" id="tab_basic">@include('Yunshop\Mryt::admin.goods.tabs.basic')</div>
                    <div class="tab-pane" id="tab_des">@include('Yunshop\Mryt::admin.goods.tabs.des')</div>
                    <div class="tab-pane" id="tab_param">@include('Yunshop\Mryt::admin.goods.tabs.param')</div>
                    <div class="tab-pane" id="tab_option">@include('Yunshop\Mryt::admin.goods.tabs.option')</div>
                    <div class="tab-pane" id="tab_dispatch">
                        {!! widget('Yunshop\Mryt\store\admin\widget\GoodsDispatchWidget', ['goods_id'=> $goods->id, 'store_id' => $store_id])!!}
                    </div>
                    @foreach(Config::get('widget.goods') as $key=>$value)
                        @if ($key == 'tab_lease_toy' || $key == 'tab_video_demand')
                            @continue
                        ...
                        @endif
                    @endforeach
                    <div class="tab-pane" id="tab_store_setting">
                        {!! widget('Yunshop\Mryt\store\admin\widget\GoodsSettingWidget', ['goods_id'=> $goods->id, 'store_id' => $store_id])!!}
                    </div>
