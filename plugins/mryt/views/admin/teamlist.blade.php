        ...
        <div class="panel panel-info">
            <div class="panel-body">
                <form action="" method="post" class="form-horizontal" role="form" id="form1">
                    ...
                    <div class="form-group col-xs-12 col-sm-2 col-md-2 col-lg-2">
                        <div class="">
                            <select name='search[award_type]' class='form-control'>
                                <option value='' @if(!$search['award_type']) selected @endif>奖励类型</option>
                                <option value='1' @if($search['award_type']=='1') selected @endif>{{ $set['team_name'] }}</option>
                                <option value='2' @if($search['award_type']=='2') selected @endif>{{ $set['thanksgiving_name'] }}</option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="panel-body">
                    <table class="table table-hover" style="overflow:visible;">
                        ...
                        <tr>
                            ...
                            <th style='text-align: center'>奖励类型</th>
                            <th style='text-align: center'>新进VIP会员</th>
                            <th style='text-align: center'>等级设置奖励金额</th>
                            <th style='text-align: center'>下级会员奖励金额</th>
                            <th style='text-align: center'>等级{{ $set['thanksgiving_name'] }}金额</th>
                        ...
                        @foreach($list as $item)
                            <tr style="text-align: center">
                                ...
                                <td style="text-align: center;">
                                    {{ $item->level_team_award_amount }}元
                                </td>
                                <td style="text-align: center;">
                                    {{ $item->lower_level_team_award_amount }}元
                                </td>
                                <td style="text-align: center;">
                                    {{ $item->level_gratitude_amount }}元
                                </td>
                                <td style="text-align: center;">
                                    {{ $item->amount }}元
                                </td>
