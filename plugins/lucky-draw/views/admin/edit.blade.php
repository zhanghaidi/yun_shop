...
<div id='re_content' v-loading="all_loading">
    ...
    <el-steps :active="active" simple style="width:90%;margin:0 auto" align-center>
        <el-step title="1、编辑活动" icon="el-icon-edit"></el-step>
    ...
    <div style="width:1100px;margin:50px auto">
        <!-- left -->
        <div class="left-con" v-if="active==0||active==1||active==2"></div>
         <!-- step 1 -->
        <el-form ref="form" :model="form" :rules="rules" label-width="20%">
                ...
                <template>
                    ...
                    <el-form-item label="活动时间" prop="time">
                        <el-row>
                            ...
                            <el-date-picker v-model="form.countdown_time[0]" type="datetime" format="yyyy-MM-dd HH:mm:ss"
                                value-format="timestamp" placeholder="选择开始时间" ></el-date-picker>
                            </el-col>
                            <el-col :span="12" style="float:none;margin-top:10px;width:80%;">
                            <el-date-picker v-model="form.countdown_time[1]" type="datetime" format="yyyy-MM-dd HH:mm:ss"
                                value-format="timestamp" placeholder="选择结束时间"></el-date-picker>
                            </el-col>
                        </el-row>
                    </el-form-item>
                    <el-form-item label="参与用户身份" prop="role_type">
                        <el-radio v-model.munber="form.role_type" :label="0" >所有用户</el-radio>
                        <el-radio v-model.munber="form.role_type" :label="1" >部分用户</el-radio>
                        <div v-if="form.role_type">
                            <el-radio v-model.munber="form.member_type" :label="0" >会员等级</el-radio>
                            <el-radio v-model.munber="form.member_type" :label="1" >推广员</el-radio>
                            <el-select v-model="form.level_id" placeholder="请选择会员等级" clearable v-if="!form.member_type" style="width:70%">
                    ...
                    <el-form-item label="购买商品" prop="goods_id">
                        ...
                        <el-button type="danger" @click="clearGoods()" v-if="form.goods_id">点击清空</el-button>
                    </el-form-item>
                    ...
                    <el-form-item label="活动类型" prop="draw_type">
                        <el-radio v-model.munber="form.draw_type" :label="0" >无限制抽签</el-radio><br>
                        <el-radio v-model.munber="form.draw_type" :label="1" >积分抽奖</el-radio>
                        <span style="font-weight:500;padding-left:20px">消耗<input type="text" style="height:25px;width:100px;" v-model="form.use_point"/>积分</span><br>
                        <el-radio v-model.munber="form.draw_type" :label="2" v-if="love_status == 1">[[love_name]]抽奖</el-radio>
                        <span style="font-weight:500;padding-left:20px" v-if="love_status == 1">消耗<input type="text" style="height:25px;width:100px;" v-model="form.use_love"/>[[love_name]]</span><br>
                    </el-form-item>
                    
                    <el-form-item label="参与次数" prop="partake_times">
                        <el-radio v-model.munber="form.partake_times" :label="0" >一天N次</el-radio><br>
                        <div v-if="form.partake_times==0">
                            <div style="font-weight:500;padding-left:20px">每人每天抽<input type="text" v-model="form.days_times" style="height:25px;width:100px;"/>次</div>
                            <div style="font-weight:500;padding-left:20px">每天分享额外抽<input type="text"  v-model="form.days_share_times" style="height:25px;width:100px;"/>次</div>
                        </div>
                        <el-radio v-model.munber="form.partake_times" :label="1" >一人N次</el-radio>
                        <div v-if="form.partake_times==1">
                            <div style="font-weight:500;padding-left:20px">每人一共可抽<input type="text" v-model="form.somebody_times" style="height:25px;width:100px;"/>次</div>
                            <div style="font-weight:500;padding-left:20px">每人分享额外抽<input type="text" v-model="form.somebody_share_times" style="height:25px;width:100px;"/>次</div>
                        </div>
                    </el-form-item>
            ...
            <el-form  :model="form" :rules="rules" label-width="20%">
                ...
                <hr style="padding:0;margin:0"/>
                <el-button type="primary" size="mini" style="margin:5px" @click="addAward()" v-if="prize_list.length<6">添加奖品</el-button>
                <!-- 表格start -->
                  <el-table :data="prize_list" style="width: 100%" v-loading="search_loading">
                        ...
                        <el-table-column label="奖品" align="center" >
                            <template slot-scope="scope">
                                ...
                                <span v-if="scope.row.type==3">[[love_name]]</span>
                        ...
                        <el-table-column label="操作" align="center">
                            <template slot-scope="scope">
                                <a @click="editAward(scope, prize_list)">编辑</a>
                                <a @click="open(scope, prize_list)">删除</a>
                            </template>
                        </el-table-column>
                    </el-table> 
                </template> 
                ...
                <template>
                    <el-form-item label="名称" prop="name">
                        <el-input v-model="form.empty_prize_name" style="width:70%"></el-input>
                    </el-form-item>
                    <el-form-item label="图片" prop="empty_prize_thumb">
                        <div class="avatar-uploader-box">
                            <el-upload class="avatar-uploader" action="{{ yzWebFullUrl('plugin.lucky-draw.admin.controllers.activity.upload') }}" accept="image/*" :show-file-list="false"  :on-success="prizeSuccess" :before-upload="prizeUpload">
                                <img v-if="form.empty_prize_thumb" :src="form.empty_prize_thumb" class="avatar">
                                <div v-if="form.empty_prize_thumb" class="avatar-tip-text">修改</div>
                                <i v-if="!form.empty_prize_thumb" class="el-icon-plus avatar-uploader-icon"></i>
                            </el-upload>
                            <i v-show="form.empty_prize_thumb" class="el-icon-circle-close"  @click="clearPrize" title="点击清除图片"></i>
                        </div>
                        <div class="tip">建议尺寸96*96像素，支持JPG、PNG、JPEG格式</div>
                    </el-form-item>
                    <el-form-item label="提示语" prop="tip">
                        <el-input v-model="form.empty_prize_prompt" style="width:70%"></el-input>
                    </el-form-item>
                    <el-form-item label="提示跳转" prop="jump_type">
                        <el-radio v-model.munber="form.jump_type" :label="0" >无跳转</el-radio>
                        <el-radio v-model.munber="form.jump_type" :label="1" >有跳转</el-radio>
                            <el-form-item label="" v-if="form.jump_type">
                                <el-input type="textarea" placeholder="请输入跳转链接，以https://开头" v-model="form.jump_link"></el-input>
                            </el-form-item>
                    ...
                    <div style="margin:10px 0">
                        <span style="font-weight:500;padding-left:20px">送<el-input type="text" style="width:100px;" v-model="form.partake_point"></el-input>积分</span>
                        <span style="font-weight:500;padding-left:20px" v-if="love_status == 1">送<el-input type="text" style="width:100px;" v-model="form.partake_love"></el-input>[[love_name]]</span>
                        <span style="font-weight:500;padding-left:20px">送<el-input type="text" style="width:100px;" v-model="form.partake_amount"></el-input>余额</span>
                    </div>
                    <el-form-item label="送优惠券">
                        <el-input style="width:300px;" v-model="partake_coupon.name" ></el-input>
                        <el-button @click="couponShow()">选择优惠券</el-button>
                    </el-form-item>
                    <el-form-item label="" prop="limit">
                        <el-checkbox v-model="form.limit">仅送给未中奖者</el-checkbox>
                    </el-form-item>
                </template>
                </el-form-item>
            </div>
                    ...
                    <el-form-item label="图片" prop="name">
                        <div class="avatar-uploader-box">
                            <el-upload class="avatar-uploader" action="{{ yzWebFullUrl('plugin.lucky-draw.admin.controllers.activity.upload') }}" accept="image/*" :show-file-list="false" :on-success="diySuccess" :before-upload="diyUpload">
                                ...
                                <div v-if="form.background" class="avatar-tip-text">修改</div>
                                <i v-if="!form.background" class="el-icon-plus avatar-uploader-icon"></i>
                            </el-upload>
                            <i v-show="form.background" class="el-icon-circle-close" @click="clearDiy" title="点击清除图片"></i>
                        </div>
                        <div class="tip">建议尺寸375x160px，支持JPG、PNG、JPEG格式</div>
                    </el-form-item>
                    <el-form-item label="商家LOGO" prop="is_logo">
                        <el-radio v-model.munber="form.is_logo" :label="0" >不展示</el-radio>
                        <el-radio v-model.munber="form.is_logo" :label="1" >展示</el-radio>
                    </el-form-item>
                    <el-form-item label="中奖名单" prop="is_award_list">
                        <el-radio v-model.munber="form.is_roster" :label="0" >不展示</el-radio>
                        <el-radio v-model.munber="form.is_roster" :label="1" >展示</el-radio>
                    </el-form-item>
                    <el-form-item label="背景色" prop="color">
                        <el-color-picker v-model="form.background_colour"></el-color-picker>
                    </el-form-item>
                    <div style="line-height:40px;padding-left:15px;font-weight:900;">活动说明</div>
                    <hr style="padding:0;margin-top:0"/>
                    <el-input type="textarea" rows="5" placeholder="请输入活动说明" style="width:90%;margin-left:5%;padding-bottom:20px;" v-model="form.content"></el-input>
                </template>
            </div>
             <div v-if="active==4">
                <div style="text-align:center">
                    ...
                    <div style="font-weight:900;font-size:30px;line-height:48px;">幸运大转盘编辑成功</div>
    ...
    <el-dialog title="编辑奖品" :visible.sync="is_edit_dialog" @close="closeEdit" class="editAwrad" :lock-scroll="true">
        <div>
            <el-form ref="edit_form" :model="edit_form" :rules="rules" label-width="20%">
                <template>
                    <el-form-item label="名称" prop="name">
                        <el-input v-model="edit_form.name" style="width:70%" placeholder="最多可填6个字" v-model="edit_form.name" :value="edit_form.name"></el-input>
                    </el-form-item>
                    <el-form-item label="奖品" prop="award_type">
                        <div>
...
<script>
    var vm = new Vue({
        ...
        data() {
            ...
            activityModel.level_id = Number(activityModel.level_id);
            activityModel.limit =  activityModel.limit==1?true:false;
            activityModel.countdown_time[0]=activityModel.countdown_time[0]*1000;
            activityModel.countdown_time[1]=activityModel.countdown_time[1]*1000;
            let prize_list = {!! $prize_list?:'{}' !!}
            return {
                love_name : love_name,
                love_status : love_status,
                prize_list:[...prize_list],
                partake_coupon:{
                    ...activityModel.has_one_coupon
                },
                coupon_show:false,
                couponSend:{},
                AwardList:[],
                ...
                goods : [],
                good : activityModel.goods_id ? '[GID:' + activityModel.has_one_goods.id + '][商品名称:' + activityModel.has_one_goods.title + ']' : '',
                imageUrl : activityModel.goods_id ? activityModel.has_one_goods.thumb : '{}',
                loading:false,     
                rules:{},
            }
        },
           ...
           next(){
               if(this.active===0){
                if(this.form.name===''){
                    ...
                    if(this.form.member_type===0&&this.form.role_ype===1){
               ...
               switch(this.form.draw_type) {
                          case 0:
                          break;
                          case 1:
                          if(this.form.use_point===''){
                            this.$message.error("请填写消耗积分");
                            return
                          }
                          if(isNaN(this.form.use_point)){
                            this.$message.error("消耗积分需为数字");
                            return
                          }
                          break;
                          case 2:
                          if(this.form.use_love===''){
                            this.$message.error(`请填写消耗${this.love_name}`);
                            return
                          }
                          if(isNaN(this.form.use_love)){
                            this.$message.error(`消耗${this.love_name}需为数字`);
                            return
                          }
                          break;
                }
                switch(this.form.partake_times){
                          case 0:
                          if(this.form.days_times===''){
                            this.$message.error("请填写每人每天抽取的次数");
                            return
                          }
                          if(isNaN(this.form.days_times)){
                            this.$message.error("每人每天抽取的次数需为数字");
                            return
                          }
                          if(this.form.days_share_times===''){
                            this.$message.error("请填写每天分享额外抽取的次数");
                            return
                          }
                          if(isNaN(this.form.days_share_times)){
                            this.$message.error("每天分享额外抽取的次数需为数字");
                            return
                          }
                          break;
                          case 1:
                          if(this.form.somebody_times===''){
                            this.$message.error("请填写每人一共可抽取的次数");
                            return
                          }
                          if(isNaN(this.form.somebody_times)){
                            this.$message.error("每人一共可抽取的次数需为数字");
                            return
                          }
                          if(this.form.somebody_share_times===''){
                            this.$message.error("请填写每人分享额外抽取的次数");
                            return
                          }
                          if(isNaN(this.form.somebody_share_times)){
                    ...
                    if(this.active===1){
                        ...
                        if(this.form.empty_prize_name===''){
                            this.$message.error("请先填写未中奖设置名称");
                            return 
                        }
                        if(this.form.empty_prize_thumb===''){
                            this.$message.error("请先传未中奖设置图片");
                            return 
                            
                        }
                        if(this.form.partake_point==''&&this.form.partake_love==''&&this.form.partake_amount==''&&(this.partake_coupon.name==''||this.partake_coupon.name==undefined)){
                            this.$message.error("请先填写参与者奖励");
                            return
                        }
                        let changeAmount=this.prize_list.map((item,index,key)=>{return Number(item.chance)})
                    ...
                    if(this.active<=2){
                        console.log(this.partake_coupon.name)
                        this.active++;
                    }
            },
            last() {
            ...
            deleteAward(scope, rows){
                ...
                this.form.prize_id.map((item,index,key)=>{
                    if(item===scope.row.id){
                        this.form.prize_id.splice(index,1);
                    }
                })
                let json={
                    id:scope.row.id
    ...
                 }
            ...
            diySuccess (res, file){
                if (res.result == 1) {
                    this.form.background = res.data.thumb_url;
    ...
                 }
            ...
            prizeSuccess (res, file){
                if (res.result == 1) {
                    this.form.empty_prize_thumb = res.data.thumb_url;
                    this.$message.success("上传成功！");
                } else {
                    this.$message.error(res.msg);
                }
    ...
                 }
            ...
            choose() {
                ...
                this.$http.post("{!! yzWebFullUrl('plugin.lucky-draw.admin.controllers.activity.addPrize') !!}",{'form_data':json}).then(response => {
                                ...
                                if (response.data.result) {
                                    ...
                                    this.prize_list.push(json);
                                    this.form.prize_id.push(json.id);
                                    this.prize_list.map((item,index,key)=>{
                                        if(item.type==1){
                                            item.award='优惠券'
                                        }
                                        if(item.type==2){
                                ...
                                if (response.data.result) {
                                    this.prize_list.map((item,index,key)=>{
                                        if(item.id===this.edit_form.id){
                                            this.prize_list.splice(index,1,this.edit_form)
                                            this.prize_list.map((item,index,key)=>{
            ...
            submit() {
                if(this.active===2){
                    ...
                    if(this.form.background_colour===''){
                    ...
                    let json={
                    name:this.form.name,
                    countdown_time:[Number(String(this.form.countdown_time[0]).substring(0,10)),Number(String(this.form.countdown_time[1]).substring(0,10))],
                    role_type:this.form.role_type,
                    member_type:this.form.member_type,
                    level_id:this.form.level_id,
                    draw_type:this.form.draw_type,
                    use_point:this.form.draw_type===1?this.form.use_point:'',
                    use_love:this.form.draw_type===2?this.form.use_love:'',
                    partake_times:this.form.partake_times,
                    days_times:this.form.partake_times===0?this.form.days_times:'',
                    days_share_times:this.form.partake_times===0?this.form.days_share_times:'',
                    somebody_times:this.form.partake_times===1?this.form.somebody_times:'',
                    somebody_share_times:this.form.partake_times===1?this.form.somebody_share_times:'',
                    prize_id:this.form.prize_id,
                    empty_prize_name:this.form.empty_prize_name,
                    empty_prize_thumb:this.form.empty_prize_thumb,
                    jump_type:this.form.jump_type,
                    jump_link:this.form.jump_type===1?this.form.jump_link:'',
                    partake_point:this.form.partake_point,
                    partake_love:this.form.partake_love,
                    partake_amount:this.form.partake_amount,
                    partake_coupon_id:this.partake_coupon.name!=''&&this.partake_coupon.name!=undefined?this.partake_coupon.id:'',
                    limit:this.form.limit===true?1:0,
                    background:this.form.background,
                    background_colour:this.form.background_colour,
                    is_logo:this.form.is_logo,
                    is_roster:this.form.is_roster,
                    content:this.form.content,   
                    empty_prize_prompt:this.form.empty_prize_prompt,
                    id:this.form.id,
                    goods_id:this.form.goods_id,
                    }
                    this.$http.post('{!! yzWebFullUrl('plugin.lucky-draw.admin.controllers.activity.edit') !!}',{'form_data':json}).then(function (response){
                        this.$message({message: "编辑活动成功",type: 'success'});
                            this.$http.post('{!! yzWebFullUrl('plugin.lucky-draw.admin.controllers.activity.getActivityById') !!}',{'id':this.form.id}).then(function (response){
            ...
            couponChoose(){
                this.coupon_show=false;
                this.partake_coupon=this.couponSend;
            },
            couponShow(){
