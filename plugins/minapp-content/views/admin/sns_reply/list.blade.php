...
<div class="rightlist">
    ...
    <div class="panel panel-defualt">
        <div class="panel-body">
            <div class="alert alert-info">帖子ID：{{$info['id']}} &nbsp;&nbsp;&nbsp;&nbsp; 内容： {{$info['content']}}</div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @elseif($value['status'] == 0)
                            <a href="{{ yzWebUrl('plugin.minapp-content.admin.sns-replys.status', ['id' => $value['id'], 'check' => '1']) }}"><span class="btn btn-success">通过审核</span></a>&nbsp;&nbsp;
                            <a href="{{ yzWebUrl('plugin.minapp-content.admin.sns-replys.status', ['id' => $value['id'], 'check' => '-1']) }}"><span class="btn btn-danger">拒绝审核</span></a>
                        @else
                            <span>已拒绝</span>
                        @endif
                        </td>
                        ...
                        <td>
                            <a href="{{ yzWebUrl('plugin.minapp-content.admin.sns-replys.post', ['id' => $value['id']]) }}"><i class="fa fa-comment-o"></i> {{$value['counts']}}</a>
                        </td>
                        <td class="text-right">
                            <a href="{{ yzWebUrl('plugin.minapp-content.admin.sns-replys.status', ['id' => $value['id'], 'flag' => 1]) }}">
