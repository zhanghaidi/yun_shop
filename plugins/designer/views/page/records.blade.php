        ...
        <div class="panel panel-info">
            ...
            <div class="panel-body">
                <form action="" method="get" class="form-horizontal" role="form">
                    ...
                    <input type="hidden" name="route" value="plugin.designer.Backend.Modules.Page.Controllers.records.index"/>
                    <div class="form-group">
                        <label class="col-xs-12 col-sm-2 col-md-2 col-lg-1 control-label">关键字</label>
                        <div class="col-sm-8 col-lg-9">
                            <input class="form-control" name="search[name]" id="" type="text" value="{{ $search['name'] or '' }}" placeholder="请输入页面名称进行搜索">
                        </div>
            ...
            <div class='panel-body'>
                <table class="table">
                    ...
                    <tr>
                        ...
                        <th style="width:15%;text-align: center;">上次修改时间</th>
                        <th style="width:15%;text-align: left;">页面名称</th>
                            ...
                            <tr pageid="{{ $list->id }}" style="height: 59px">
                                ...
                                <td style="text-align: left;">
                                    @if(in_array(1, $list->page_type_cast))
                                        <label class='label label-success'>公众号首页</label>
                                    @endif
                                    @if(in_array(2, $list->page_type_cast))
                                        <label class='label label-danger'>小程序首页</label>
                                    @endif
                                    @if(in_array(7, $list->page_type_cast))
                                        <label class='label label-primary'>APP首页</label>
                                    @endif
                                    @if(in_array(8, $list->page_type_cast))
                                        <label class='label label-info'>支付宝首页</label>
                                    @endif
                                    @if(in_array(5, $list->page_type_cast))
                                        <label class='label label-warning'>WAP首页</label>
                                    @endif
                                    @if(in_array(0, $list->page_type_cast))
                                        <label class='label label-default'>其他页面</label>
                                    @endif
                                    @if(in_array(9, $list->page_type_cast))
                                        <label class='label label-danger'>小程序（原生版）</label>
                                    @endif
                                    @if(in_array(10, $list->page_type_cast))
                                        <label class='label label-default'>小程序其他页面（原生版）</label>
                                    @endif
                                </td>
                                ...
                                <td style=" text-align:  center;position:relative">
                                    ...
                                    @if(in_array(9, $list->page_type_cast) || in_array(10, $list->page_type_cast))
                                        <a href="{{ yzWebUrl('plugin.designer.admin.list.update', array('id' => $list->id, 'page_type' => 9)) }}">编辑</a> -
                                    @else
                                        <a href="{{ yzWebUrl('plugin.designer.admin.list.update', array('id' => $list->id)) }}">编辑</a> -
                                    @endif
                                    <a href="javascript:;" onclick="delpage({{ $list->id }})">删除</a> -

                                    @if(in_array(9, $list->page_type_cast) || in_array(10, $list->page_type_cast))
                                        <a href="javascript:;" data-url="/packageC/diy/diy/diy?page_id={{ $list->id }}" title="复制链接" class="js-clip">复制链接</a>-
                                    @else
                                        <a href="javascript:;" url="{{ yzAppFullUrl('diy', array('page_id' => $list->id)) }}" title="复制链接" class="fuzhi{{$list->id}} btns{{$list->id}}">复制链接</a>- 
                                        <script type="text/javascript">
                                            var content{{$list->id}} = $(".fuzhi{{$list->id}}").attr('url');
                                            var clipboard = new Clipboard('.btns{{$list->id}}', {
                                                text: function() { 
                                    ...
                                    @endif
                                    <a href="{{ yzWebUrl('plugin.designer.Backend.Modules.Page.Controllers.copy.index', array('id' => $list->id)) }}">复制页面</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
                    ...
                    <tr>
                        <td colspan="8">
                            ...
                            <span>Tips:同一个类型仅允许设置一个默认首页，其他页面为自定义页面</span>
                        </td>
                    </tr>
                    <tr>
