...
* 首次访问回复，设置到setting里
*/
class WelcomeAutoReplyController extends BaseController
{
    public function index()
    {
        ...
        return view('Yunshop\Wechat::admin.reply.welcome', [
    ...
    public function add()
    {
        ...
        Setting::set('wechat.reply.welcome_keywords_id', $keywords_id);
        $id = Setting::get('wechat.reply.welcome_keywords_id');
        if (!empty($id)) {
    ...
    {
        Setting::set('wechat.reply.welcome_keywords_id', 0);
        $id = Setting::get('wechat.reply.welcome_keywords_id');
        if (!empty($id)) {
