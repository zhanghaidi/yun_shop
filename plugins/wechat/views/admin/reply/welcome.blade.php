    ...
    <div id="app" v-loading="all_loading">
        ...
        <el-tabs v-model="activeName" type="card" @tab-click="handleClick" v-loading="loading">
            ...
            <el-card shadow="always" style="background:#f5f5f9;">
                    <i class="el-icon-info" style="color:#409EFF;margin:0 10px;"></i>用户关注公众号时，发送的欢迎信息。
                </el-card>
                <div>
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            delKeyword() {
                ...
                that.$http.get("{!! yzWebFullUrl('plugin.wechat.admin.reply.controller.welcome-auto-reply.delete') !!}",{}).then(response => {
                    console.log(response);
                    if(response.data.result==1){
                        that.keyword_list = response.data.data;
            ...
            submit(){
                ...
                that.all_loading = true;
                console.log(this.data);
                that.$http.post("{!! yzWebFullUrl('plugin.wechat.admin.reply.controller.welcome-auto-reply.add') !!}",{keywords_id:that.data.id}).then(response => {
                    console.log(response);
                    if(response.data.result==1){
                        that.keyword_list = response.data.data;
