...
namespace Yunshop\AlipayOnekeyLogin\admin;
...
class SetController extends BaseController
{
	...
	public function index()
	{
		...
		if ($alipaySet) {
			if (!isset($alipaySet['app']) && !empty($set['app'])) {
				...
				if (Setting::set('plugin.alipay_onekey_login', $set)) {
                	return $this->message('设置成功', Url::absoluteWeb('plugin.alipay-onekey-login.admin.set.index'));
            	}
			} else {
				$alipaySet = $this->verification($alipaySet);
				if (Setting::set('plugin.alipay_onekey_login', $alipaySet)) {
					//开启强制绑定手机
					$this->openBindMobile();
                	return $this->message('设置成功', Url::absoluteWeb('plugin.alipay-onekey-login.admin.set.index'));
            	}
		...
		return view('Yunshop\AlipayOnekeyLogin::admin.set', [
			'setdata' => $set,
		])->render();
	}
