...
* create 2018/6/18 
*/
class WeChatUserService extends MemberInfoService
{
    protected $wechat_sign = 'app';

	public function updateMember($old_member, $new_member)
	{

		$wechat_user = McMappingFans::uniacid()->where('uid', $new_member->uid)->first();
        ...
        $unique_info = MemberUniqueModel::uniacid()->where('member_id', $wechat_user->uid)->first();
        ...
        try {
            ...
            $wechat_user->uid = $old_member->uid;

            if(!$wechat_user->save()) {
                throw new Exception("mc_mapping_fans表信息修改失败");
            }
            if ($unique_info) {
...
//            \Log::debug('微信'.$this->wechat_sign.'同步会员数据uid:'.$new_member->uid.'失败');
    ...
    public function old($old_member, $new_member)
    {
        ...
        if (!is_null(McMappingFans::uniacid()->where('uid', $old_member->uid)->first())) {
