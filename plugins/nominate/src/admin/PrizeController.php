...
class PrizeController extends BaseController
{
    public function index()
    {
        ...
        $list = NominateBonus::getList($search, NominateBonus::NOMINATE_PRIZE)
            ->orderBy('id', 'desc')
            ->paginate();
        $amountTotal = NominateBonus::getList($search, NominateBonus::NOMINATE_PRIZE)
            ->sum('amount');
        $pager  = PaginationHelper::show($list->total(), $list->currentPage(), $list->perPage());
        $set = \Setting::get('plugin.nominate');

        return view('Yunshop\Nominate::prize.index', [
            'list' => $list,
            'pager' => $pager,
    ...
    {
        $search = request()->search;
        $builder = NominateBonus::getList($search, NominateBonus::NOMINATE_PRIZE)->orderBy('id', 'desc');
        $export_page = request()->export_page ? request()->export_page : 1;
        $export_model = new ExportService($builder, $export_page);
        $file_name = date('Ymdhis', time()) . '推荐-直推奖导出';
        ...
        $export_data[0] = ['ID', '时间', '会员ID', '会员昵称', '会员姓名', '会员手机号','会员等级','新进VIP会员ID','新进VIP会员昵称',
            '新进VIP会员名字', '新进VIP会员手机号', '直推奖金额', '奖励发放状态'];
