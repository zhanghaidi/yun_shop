    ...
    public function up()
    {
        if (!Schema::hasTable('yz_exam_examination_content')) {
            Schema::create('yz_exam_examination_content', function (Blueprint $table) {
                ...
                $table->string('share_title_after', 200)->default('')->comment('答题后分享标题');
                $table->string('share_describe_after', 200)->default('')->comment('答题后分享描述');
                $table->string('share_image_after', 200)->default('')->comment('答题后分享图片URL');
