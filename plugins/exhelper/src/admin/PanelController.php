...
//    	}
	...
	{
        $list = ExhelperPanel::uniacid()->orderby('id', 'desc')->paginate(10);

        $pager = PaginationHelper::show($list->total(), $list->currentPage(), $list->perPage());

		return view('Yunshop\Exhelper::admin.panel_list', [
	...
	{        
        if(request()->isMethod('post')){
	        ...
	        $info =\Yunshop::request()['user'];

	        $info['uniacid'] = \YunShop::app()->uniacid;
	        
	        if ($info['isself'] == 1) {
	        	
	        	$info['begin_time'] = $info['date'].' '.$info['begin_time'].':00';
			
				$info['end_time'] = $info['date'].' '.$info['end_time'].':00';	
			...
			$exhelperPanel = new ExhelperPanel;

			$validator = $exhelperPanel->validator($info);
	        
	        if ($validator->fails()) {

	           return $this->message($validator->messages()->first());
	        
	        } else {
	        	
	        	if ($info['isdefault'] == 1) {
	        		//检查是否有默认的模板存在
	        		$check = ExhelperPanel::uniacid()->update(['isdefault'=> 0]);
	        		
	        		if (!$check) {
	        			return $this->message('设置默认失败, 请重新选择');
	        		}
	        	}
				//添加
				$exhelperPanel->fill($info);

				$exhelperPanel->save();
				
				return $this->message('添加成功', Url::absoluteWeb('plugin.exhelper.admin.panel.index'));
			}

	    }

	    return view('Yunshop\Exhelper::admin.panel_detail', ['item'=>$exhelperPanel, 'company'=>$this->company]);
	}
	...
	public function edit()
	{
		...
		if (empty($id) ) {
			...
			return $this->message('请选择模板', Url::absoluteWeb('plugin.exhelper.admin.panel.index'), 'error');
		}

		$item = ExhelperPanel::find($id);

		if (!$item) {
			
			return $this->message('该电子面单不存在', Url::absoluteWeb('plugin.exhelper.admin.panel.index'), 'error');
		} 
		...
		if (request()->isMethod('post')) {
			...
			$exhelper = new ExhelperPanel();

			$validator = $exhelper->validator($info);
			// $validator = Validator::make($info, $exhelper->rules(), $exhelper->messages());
			// dd($validator);
			if ($validator->fails()) {
	           ...
	           return $this->message($validator->messages()->first());
				...
				if ($info['isdefault'] == 1) {
	        		//检查是否有默认的模板存在
	        		ExhelperPanel::uniacid()->update(['isdefault'=> 0]);
	        		
	        		$check = ExhelperPanel::uniacid()->where('id',$id)->update(['isdefault'=> 1]);
				...
				$item->where('id',$id)->update($info);
				
				return $this->message('修改成功', Url::absoluteWeb('plugin.exhelper.admin.panel.index'));
			}
		}
		
		return view('Yunshop\Exhelper::admin.panel_detail', [
			'item' => $item,
			'company'=>$this->company
		]);
	}
	...
	public function destory()
	{
		...
		$data = ExhelperPanel::find($id);

		if (!$data) {

			return $this->message('无该面单存在', Url::absoluteWeb('plugin.exhelper.admin.panel.index'), 'error');
		}

		$data->delete();
        
        return $this->message('删除成功', Url::absoluteWeb('plugin.exhelper.admin.panel.index'));
	}
	...
	public function isDefault()
    {
        ...
        if (!$id) {
            return $this->message('参数错误', Url::absoluteWeb('plugin.exhelper.admin.panel.index'), 'error');
        }
        $model = ExhelperPanel::find($id);
        if (!$model) {
            return $this->message('未找到数据', Url::absoluteWeb('plugin.exhelper.admin.panel.index'), 'error');
        }
        if ($model->isdefault == 1) {
            return $this->message('已经设置为默认', Url::absoluteWeb('plugin.exhelper.admin.panel.index'), 'error');
        }
      
        ExhelperPanel::uniacid()->update(['isdefault'=>0]);

        $model->update(['isdefault'=>1]);

        return $this->message('设置默认成功', Url::absoluteWeb('plugin.exhelper.admin.panel.index'));
    }
    ...
    public function test()
    {
    	...
    	if (!$ordersn || !Order::where('order_sn', $ordersn)->first()) {
    	...
    	$template_model = ElectronicTemplate::uniacid()->where('order_sn',$ordersn)->first();
    	if ($template_model){
            $panelsetting = ExhelperPanel::uniacid()->where('isdefault',1)->first()->toArray();
            return json_encode(array('result'=>'success', 'resp'=>['data'=>$panelsetting, 'html'=>$template_model['print_template']]));
        }
    ...
    */
    public function beginHandle($order)
    {
    	...
    	\Log::info('-=-=add=-=-', $add);
    	$send = SendUser::uniacid()->where('isdefault', 1)->first()->toArray();
    	\Log::info('-=-=send=-=-', $send);
...
//			}
		...
		foreach ($order['has_many_order_goods'] as $k => $v) {
            ...
            $goos_total .= $v['title'].'x'.$v['total'].'</br>';
		}
		// dd($commodity);
...
//        }
//        if ($panel['exhelper_style'] == 'YTO'){
//            $data['TemplateSize'] = 18001;
//        }
        //取模板样式传到快递鸟
        $data['TemplateSize'] =  strstr($panel['panel_style'],'mm') ? '' : $panel['panel_style']; //$this->setTemplateSize($panel['exhelper_style']);
//        dd($data['TemplateSize'],strstr($panel['panel_style'],'mm'));
		if ($panel->isself == 1) {
			//上门揽件时间
    ...
    public function getCompanyPanel($data, $url, $order_id,$order_sn)
    {
		...
		if($result['ResultCode'] == '100' && $result['Success'] == 'true') {
			...
			return json_encode(array('result'=>'success', 'resp'=>['data'=>$panelsetting, 'html'=>$result['PrintTemplate']]));
     ...
     * 更新快递鸟电子面单最新模板
     */
    public function setTemplateSize($exhelper_style)
    {
        $templateSize = '';
        switch ($exhelper_style){
            case 'YTO' : $templateSize = 18001;  break;
            case 'SF' : $templateSize = 15001;  break;
            case 'ZTO' : $templateSize = 180;  break;
            case 'ZJS' : $templateSize = 120;  break;
            case 'YZPY' : $templateSize = 180;  break;
            case 'SURE' : $templateSize = 180;  break;
            case 'STO' : $templateSize = 18003;  break;
            case 'JD' : $templateSize = 110;  break;
            case 'HTKY' : $templateSize = 180;  break;
            case 'DBL' : $templateSize = 18001;  break;
            default : $templateSize = '';break;
        }
