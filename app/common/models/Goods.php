...
//                                ->where('yz_goods_category.category_id', $value);
    ...
    {
        // 库存,销量缓存
        $cache = \Cache::get("goods.{$this->id}.{$key}");
        if (!isset($cache)) {
            $cache = $this->attributes[$key];
            // 过期时间一个月
            \Cache::put("goods.{$this->id}.{$key}", $cache, 43200);
        }
        return $cache;
    }
    ...
    {
        if(SiteSettingFacades::get('base.stock_cache')){
            // 库存缓存
    ...
    public function getRealSalesAttribute()
    {
        if(SiteSettingFacades::get('base.stock_cache')){
            // 销量缓存
            return $this->getCache('real_sales');
        }
        return $this->attributes['real_sales'];
    }

    public function getShowSalesAttribute()
    {
        if(SiteSettingFacades::get('base.stock_cache')){
            // 销量缓存
            return $this->getCache('show_sales');
        }
        return $this->attributes['show_sales'];
    }
    ...
    public function reduceStock($num)
    {
        if ($this->reduce_stock_method != 2) {
            ...
            \Cache::decrement("goods.{$this->id}.stock", $num);
        }
    }
    ...
    {
        if ($this->getCache('real_sales')) {
            \Cache::increment("goods.{$this->id}.real_sales", $num);
        }
        if ($this->getCache('show_sales')) {
            \Cache::increment("goods.{$this->id}.show_sales", $num);
        }
    }
    public function addDBSales($num)
    {
...
//            }
    ...
    public function save()
    {
        $result = parent::save();
        // 清除库存缓存
        if(SiteSettingFacades::get('base.stock_cache') && $result){
            \Cache::forget("goods.{$this->id}.stock");
            \Cache::forget("goods.{$this->id}.real_sales");
            \Cache::forget("goods.{$this->id}.show_sales");
        }
        return $result;
    }
}
