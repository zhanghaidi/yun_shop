...
//            $memberService->chkAccount(\YunShop::app()->getMemberId());
            ...
            if ($old_member) {
                ...
                if($type == 1){
                    if (empty($member_merge_set['is_merge_save_level'])) {
                        DB::transaction(function () use (&$member_model, $memberinfo_model, $uid, $mobile, $salt, $password) {
                            //同步绑定已存在的手机号
                            if (!empty($memberinfo_model) && ($memberinfo_model->createtime < $member_model->createtime)) {
                                $this->phoneMemberSave($uid, $memberinfo_model, $member_model, $mobile, $password, 1);
                            } elseif (!empty($memberinfo_model) && ($memberinfo_model->createtime > $member_model->createtime)) {
                                $this->fansMemberSave($uid, $memberinfo_model, $member_model, $mobile);
                            }
                        });
                    } elseif ($member_merge_set['is_merge_save_level'] == 1) {
                        //同步绑定已存在的手机号
                        if (!empty($memberinfo_model)) {
                            $this->phoneMemberDataSave($uid, $memberinfo_model, $member_model, $mobile, $password, 1);
                        }
                ...
                if ($type == 2) {
                    if (empty($member_merge_set['is_merge_save_level'])) {
                        DB::transaction(function () use (&$member_model, $memberinfo_model, $uid, $mobile, $salt, $password) {
                            //同步绑定已存在的手机号
                            if (!empty($memberinfo_model) && ($memberinfo_model->createtime < $member_model->createtime)) {
                                $this->phoneMemberSave($uid, $memberinfo_model, $member_model, $mobile, $password, 2);
                            } elseif (!empty($memberinfo_model) && ($memberinfo_model->createtime > $member_model->createtime)) {
                                $this->fansMemberSave($uid, $memberinfo_model, $member_model, $mobile);
                            }
                        });
                        ...
                        //同步绑定已存在的手机号
                        if (!empty($memberinfo_model)) {
                            $this->phoneMemberDataSave($uid, $memberinfo_model, $member_model, $mobile, $password, 2);
                        }
                    } elseif ($member_merge_set['is_merge_save_level'] == 2) {
                        //公众号
                        $fans = McMappingFans::getFansById($uid);
    ...
    public function phoneMemberSave($uid, $memberinfo_model, $member_model, $mobile, $password, $type)
    {
        ...
        if ($type == 2) {
            //更新fans表的uid字段  小程序
            $fansinfo = MemberMiniAppModel::getFansById($uid);
            $fansinfo->member_id = $mc_uid;
        } else {
            //更新fans表的uid字段
    ...
    public function fansMemberSave($uid, $memberinfo_model, $member_model, $mobile)
    {
        //app注册的会员信息id
        $mc_uid = $memberinfo_model['uid'];
        //app注册的会员的余额 积分
    ...
    public function phoneMemberDataSave($uid, $memberinfo_model, $member_model, $mobile, $password, $type)
    {
        //app注册的会员信息id
        $mc_uid = $memberinfo_model['uid'];
        //微信注册的会员的余额 积分
        ...
        if ($type == 2) {
            ...
            //公众号
            $fans_member = McMappingFans::getFansById($uid);
            if (!empty($fans_member)) {
                McMappingFans::where('uid', $uid)->update(['uid' => $mc_uid]);
            }
        } else {
            //更新fans表的uid字段
            $fansinfo = McMappingFans::getFansById($uid);
