...
class JuShuiTanController extends ApiController
{
    ...
    {
        $orders = Order::where(['status' => 1, 'jushuitan_status' => 0])->where('pay_time', '<=', time()-1800)->with('address', 'hasManyOrderGoods', 'hasOneOrderPay')
            ->orderBy('create_time', 'ASC')->chunk(100, function ($orders) {
                foreach ($orders as $order) {
                    ...
                    $params = array([
                        ...
                        'remark' => $order->has_one_order_remark->remark, //string 卖家备注 长度<=150；可更新
