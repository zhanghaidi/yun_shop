...
class ResetpwdController extends BaseController
{
    ...
    public function sendSmsV2($mobile, $code, $state, $templateType = 'reg', $sms_type=2)
    {
        ...
        if ($sms['type'] == 1) {
            ...
            //$type为1是注册，2是找回密码
            if (!is_null($sms_type) && $sms_type == 2) {
                $response = $ssender->sendWithParam('86', $mobile, $sms['tx_templateCodeForget'],
                    [$code], $sms['tx_signname'], "", "");  // 签名参数不能为空串
            }elseif (!is_null($sms_type) && $sms_type == 'login'){
                $response = $ssender->sendWithParam('86', $mobile, $sms['tx_templateCodeLogin'],
                    [$code], $sms['tx_signname'], "", "");  // 签名参数不能为空串
            }else{
                $response = $ssender->sendWithParam('86', $mobile, $sms['tx_templateCode'],
                    [$code], $sms['tx_signname'], "", "");  // 签名参数不能为空串
            }
            $response = json_decode($response);
            if ($response->result == 0 && $response->errmsg == 'OK') {
                \Log::debug($response. '腾讯云短信发送成功');
                return $this->successJson('获取成功');
            } else {
                \Log::debug($response. '腾讯云短信发送失败' . $response->errmsg);
